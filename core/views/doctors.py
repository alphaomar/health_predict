...
class DoctorDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Fetch languages spoken by the doctor (assuming it's a ManyToManyField or similar)
        languages_spoken = self.object.languages_spoken.all()

        # Add the related doctors, reviews, appointments, and languages spoken to the context
        context['related_doctors'] = related_doctors
        context['reviews'] = reviews
        context['appointments'] = appointments
        context['languages_spoken'] = languages_spoken
...
#     success_url = reverse_lazy('appointments:appointment_success')  # URL to redirect upon success
#
...
#             # Authenticated user: Use the patient's profile from the logged-in user
...
#             )
#         else:
#             # Anonymous user: Create an appointment with anonymous info
#             appointment = Appointment.objects.create(
#                 doctor=doctor,
#                 appointment_date=form.cleaned_data['appointment_date'],
#                 appointment_method=form.cleaned_data['appointment_method'],
#                 anonymous_name=form.cleaned_data.get('name'),
#                 anonymous_phone=form.cleaned_data.get('phone_number'),
#             )
#
#         # If the form submission is via AJAX, return JSON response
